...
class UserRelationship:
    ...
    def is_accepted(self) -> bool:
        """Return True iff this relationship has ACCEPTED status."""
        return self.status == STATUS_ACCEPTED

    @property
    def is_accepted_friend(self) -> bool:
        """Return True iff FRIEND + ACCEPTED — the only type granting private post access."""
        return self.relationship_type == RELATIONSHIP_TYPE_FRIEND and self.is_accepted

    @property
    def is_block(self) -> bool:
        """Return True iff this is a BLOCK relationship (regardless of status)."""
